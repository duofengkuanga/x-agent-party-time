...
`explain` returns intra-procedural taint findings (`TAINTED` edges) recorded by `gitnexus analyze --index-only --pdg` — each with a sink category (command-injection, code-injection, path-traversal, sql-injection, xss), source/sink lines, and the ordered hop path with the variable carried on each hop.
