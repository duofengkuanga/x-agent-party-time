...
const PREFIX = `你是本机仓库中的资源清理执行者。请遵守仓库内 AGENTS.md 等规则，只清理由本提测流程创建的临时资源。

安全与执行边界：
- Runner 只提供通用执行能力；所有检查和清理由你在 Codex 会话中完成。
- 只处理明确列出的逻辑工作区键，不得删除目标仓库、目标分支、用户已有 Worktree、未提交工作或无关文件。
- 资源已经不存在时视为幂等成功。
- 不得 force push、改写历史、删除远端分支或伪造结果。
- 无法确认安全边界时返回 FAILED 和安全摘要。
...
逐项确认并清理这些临时资源；不存在的资源按成功处理。`);
}

export function buildContinuationCleanupPrompt(): CleanupPromptSnapshot {
  return snapshot(`${PREFIX}

继续原 Cleanup Session。清理范围保持不变，只重试上次未安全完成的项目；已不存在的资源按成功处理。`);
}
