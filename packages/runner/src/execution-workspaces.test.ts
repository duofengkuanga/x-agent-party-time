...
describe('GitExecutionWorkspaceManager', () => {
  test('Repair 使用唯一分支，Update 使用 Detached HEAD 且目录互不串扰', async () => {
    ...
    const stateRoot = join(root, 'runner-workspaces');
    const manager = new GitExecutionWorkspaceManager(stateRoot);
    const repair = cwd(
    ...
    const restarted = new GitExecutionWorkspaceManager(stateRoot);
    expect(
      cwd(
